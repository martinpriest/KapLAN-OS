...
if ($_SESSION['userActive'] == true) {
    ...
    if($_SESSION['idFamily'] == $deviceGroup->getIdFamily() && $_SESSION['deviceGroupPermission'] == 1) {
        ...
        $stmt = $devices->getAllRelayDevices(); //NAPISAC TA FUNKCJE W MODELU
        $num = $stmt->rowCount();
        if($num > 0) {
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                if($idDeviceType == 1) $deviceMeasurement->setIdMeasurementType(5)->create();
                else if($idDeviceType == 2) {
                    $deviceMeasurement->setIdMeasurementType(5)->create();
                    $deviceMeasurement->setIdMeasurementType(6)->create();
                } else if($idDeviceType == 3) {
                    $deviceMeasurement->setIdMeasurementType(5)->create();
                    $deviceMeasurement->setIdMeasurementType(6)->create();
                    $deviceMeasurement->setIdMeasurementType(7)->create();
                    $deviceMeasurement->setIdMeasurementType(8)->create();
                }
            }
